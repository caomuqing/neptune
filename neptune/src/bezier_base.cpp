...
Bernstein::Bernstein(double min_order_ = 3.0)
{
	...
	{	
		...
		MatrixXd Q_l, Q_u, Q_arc, Q;  // Cost Hessian matrix in each block of the objective. No scale, only meta elements .
		...
		else
		{
			...
			Q = (min_order - min_order_l) * Q_u + (min_order_u - min_order) * Q_l;
		}
