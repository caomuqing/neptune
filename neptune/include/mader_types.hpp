...
typedef Eigen::Matrix<double, 2, Eigen::Dynamic> Polygon_Std;
typedef Eigen::Matrix<double, 2, Eigen::Dynamic> PointsofInterval;

typedef std::vector<mt::Polyhedron_Std> ConvexHullsOfCurve_Std;
typedef std::vector<mt::Polygon_Std> ConvexHullsOfCurve_Std2d;
typedef std::vector<mt::PointsofInterval> SampledPointsofIntervals;

typedef std::vector<mt::ConvexHullsOfCurve_Std> ConvexHullsOfCurves_Std;
typedef std::vector<mt::ConvexHullsOfCurve_Std2d> ConvexHullsOfCurves_Std2d;
typedef std::vector<mt::SampledPointsofIntervals> SampledPointsofCurves;
...
struct basisConverter
{
     ...
     -6.6792587327074839365081970754545,  8.1917862965657040064115790301003, -1.5981560640774179482548333908198,                  0.085628500219197656306846511142794,
      3.4416308968564117698463178385282, -3.3353445427890959784633650997421, 0.80808514571348655231020075007109, -0.0000000000000000084567769453869345852581318467855;
       ...
       A_vel_mv_rest << 
        ...
        1.50000000000000,-2.36602540378444,0.933012701892219,
        -3,3,0,
        1.50000000000000,-0.633974596215561,0.0669872981077807;
 ...
 -0.07734834561012298,  0.1666641326312795,     1.07734045429237;
  ...
  Eigen::Matrix<double, 3, 3> getAvelrestMinvo()
  {
    return A_vel_mv_rest;
  }  
...
struct parameters
{
  ...
  double sample_T=1.0;
  int num_of_agents = 4;
  double tetherLength = 100;
  std::vector<Eigen::Vector2d> pb;
  int id;
  int num_sample_per_interval = 1;
  double T_span = 0.5;
  double safe_factor = 1.0;
  double rad_term_pos = 0.5;
  bool use_linear_constraints = true;
  int num_of_static_obst = 0;
  bool use_not_reaching_soln = true;
  bool move_when_goal_occupied  =false;
  bool enable_entangle_check = true;
