...
class Bernstein
{
	private:
		...
		std::vector<Eigen::MatrixXd> MQMList_l, MQMList_u, MList, MQMList_arc, FMList, MQMList;
		std::vector<Eigen::VectorXd> CList, CvList, CaList, CjList;

		Eigen::MatrixXd FM, MQM_l, MQM_u, M, MQM_arc, MQM;
	...
	public:
		...
		std::vector<Eigen::MatrixXd> getMQMs(){ return MQMList; };		
